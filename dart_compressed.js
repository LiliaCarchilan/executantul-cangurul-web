...
;(function(root, factory) {
    ...
    Blockly.Dart.init = function(a) {
        ...
        for (var b = [], c = Blockly.Variables.allDeveloperVariables(a), d = 0; d < c.length; d++)
            b.push(Blockly.Dart.variableDB_.getName(c[d], Blockly.Names.DEVELOPER_VARIABLE_TYPE));
        a = Blockly.Variables.allUsedVarModels(a);
        for (d = 0; d < a.length; d++)
            b.push(Blockly.Dart.variableDB_.getName(a[d].getId(), Blockly.VARIABLE_CATEGORY_NAME));
    ...
    Blockly.Dart.scrub_ = function(a, b, c) {
        ...
        if (!a.outputConnection || !a.outputConnection.targetConnection) {
            ...
            e && (e = Blockly.utils.string.wrap(e, Blockly.Dart.COMMENT_WRAP - 3),
            d = a.getProcedureDef ? d + Blockly.Dart.prefixLines(e + "\n", "/// ") : d + Blockly.Dart.prefixLines(e + "\n", "// "));
            for (var f = 0; f < a.inputList.length; f++)
                a.inputList[f].type == Blockly.INPUT_VALUE && (e = a.inputList[f].connection.targetBlock()) && (e = Blockly.Dart.allNestedComments(e)) && (d += Blockly.Dart.prefixLines(e, "// "))
        }
    ...
    Blockly.Dart.lists_getIndex = function(a) {
        function b() {
            var f = Blockly.Dart.variableDB_.getDistinctName("tmp_list", Blockly.VARIABLE_CATEGORY_NAME)
              , g = "List " + f + " = " + e + ";\n";
            e = f;
            return g
        }
        ...
        if (("RANDOM" != d || "REMOVE" != c) && "FROM_END" != d || e.match(/^\w+$/))
            ...
            case "FROM_END":
                ...
                if ("GET" == c)
                    return [e + "[" + e + ".length - " + d + "]", Blockly.Dart.ORDER_UNARY_POSTFIX];
            ...
            case "RANDOM":
                Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;";
                if ("REMOVE" == c)
                    return c = Blockly.Dart.variableDB_.getDistinctName("tmp_x", Blockly.VARIABLE_CATEGORY_NAME),
                    "int " + c + " = new Math.Random().nextInt(" + e + ".length);\n" + (e + ".removeAt(" + c + ");\n");
                if ("GET" == c)
                    return c = Blockly.Dart.provideFunction_("lists_get_random_item", ["dynamic " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List my_list) {", "  int x = new Math.Random().nextInt(my_list.length);", "  return my_list[x];", "}"]),
                    [c + "(" + e + ")", Blockly.Dart.ORDER_UNARY_POSTFIX];
                if ("GET_REMOVE" == c)
                    return c = Blockly.Dart.provideFunction_("lists_remove_random_item", ["dynamic " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List my_list) {", "  int x = new Math.Random().nextInt(my_list.length);", "  return my_list.removeAt(x);", "}"]),
                    [c + "(" + e + ")", Blockly.Dart.ORDER_UNARY_POSTFIX]
            }
        else {
            if ("RANDOM" == d)
                return Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;",
                a = b(),
                c = Blockly.Dart.variableDB_.getDistinctName("tmp_x", Blockly.VARIABLE_CATEGORY_NAME),
                a + ("int " + c + " = new Math.Random().nextInt(" + e + ".length);\n") + (e + ".removeAt(" + c + ");\n");
            if ("REMOVE" == c)
                return d = Blockly.Dart.getAdjusted(a, "AT", 1, !1, Blockly.Dart.ORDER_ADDITIVE),
                a = b(),
                a += e + ".removeAt(" + e + ".length - " + d + ");\n";
            if ("GET" == c)
                return d = Blockly.Dart.getAdjusted(a, "AT", 1),
                c = Blockly.Dart.provideFunction_("lists_get_from_end", ["dynamic " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List my_list, num x) {", "  x = my_list.length - x;", "  return my_list[x];", "}"]),
                [c + "(" + e + ", " + d + ")", Blockly.Dart.ORDER_UNARY_POSTFIX];
            if ("GET_REMOVE" == c)
                return d = Blockly.Dart.getAdjusted(a, "AT", 1),
                c = Blockly.Dart.provideFunction_("lists_remove_from_end", ["dynamic " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(List my_list, num x) {", "  x = my_list.length - x;", "  return my_list.removeAt(x);", "}"]),
                [c + "(" + e + ", " + d + ")", Blockly.Dart.ORDER_UNARY_POSTFIX]
        }
    ...
    Blockly.Dart.lists_setIndex = function(a) {
        function b() {
            if (e.match(/^\w+$/))
                return "";
            var g = Blockly.Dart.variableDB_.getDistinctName("tmp_list", Blockly.VARIABLE_CATEGORY_NAME)
              , k = "List " + g + " = " + e + ";\n";
            e = g;
            return k
        }
    ...
    Blockly.Dart.controls_repeat_ext = function(a) {
        ...
        a = "";
        var d = Blockly.Dart.variableDB_.getDistinctName("count", Blockly.VARIABLE_CATEGORY_NAME)
          , e = b;
        b.match(/^\w+$/) || Blockly.isNumber(b) || (e = Blockly.Dart.variableDB_.getDistinctName("repeat_end", Blockly.VARIABLE_CATEGORY_NAME),
        a += "var " + e + " = " + b + ";\n");
    ...
    Blockly.Dart.controls_for = function(a) {
        ...
        if (Blockly.isNumber(c) && Blockly.isNumber(d) && Blockly.isNumber(e)) {
            ...
            a = "",
            g = c,
            c.match(/^\w+$/) || Blockly.isNumber(c) || (g = Blockly.Dart.variableDB_.getDistinctName(b + "_start", Blockly.VARIABLE_CATEGORY_NAME),
            a += "var " + g + " = " + c + ";\n"),
            c = d,
            d.match(/^\w+$/) || Blockly.isNumber(d) || (c = Blockly.Dart.variableDB_.getDistinctName(b + "_end", Blockly.VARIABLE_CATEGORY_NAME),
            a += "var " + c + " = " + d + ";\n"),
            d = Blockly.Dart.variableDB_.getDistinctName(b + "_inc", Blockly.VARIABLE_CATEGORY_NAME),
            a += "num " + d + " = ",
    ...
    Blockly.Dart.math_number = function(a) {
        ...
        if (Infinity == a) {
            ...
            -Infinity == a ? (a = "-double.infinity",
            b = Blockly.Dart.ORDER_UNARY_PREFIX) : b = 0 > a ? Blockly.Dart.ORDER_UNARY_PREFIX : Blockly.Dart.ORDER_ATOMIC;
        return [a, b]
    }
    ;
    Blockly.Dart.math_arithmetic = function(a) {
        ...
        return c ? [d + c + a, b] : (Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;",
    ...
    Blockly.Dart.math_single = function(a) {
        ...
        if ("NEG" == b)
            return a = Blockly.Dart.valueToCode(a, "NUM", Blockly.Dart.ORDER_UNARY_PREFIX) || "0",
            "-" == a[0] && (a = " " + a),
        ...
        if (c)
            return [c, Blockly.Dart.ORDER_UNARY_POSTFIX];
        switch (b) {
        case "LOG10":
    ...
    Blockly.Dart.procedures_defreturn = function(a) {
        var b = Blockly.Dart.variableDB_.getName(a.getFieldValue("NAME"), Blockly.PROCEDURE_CATEGORY_NAME)
          , c = "";
        Blockly.Dart.STATEMENT_PREFIX && (c += Blockly.Dart.injectId(Blockly.Dart.STATEMENT_PREFIX, a));
        ...
        for (var k = f ? "dynamic" : "void", l = [], m = a.getVars(), h = 0; h < m.length; h++)
            l[h] = Blockly.Dart.variableDB_.getName(m[h], Blockly.VARIABLE_CATEGORY_NAME);
        c = k + " " + b + "(" + l.join(", ") + ") {\n" + c + d + e + g + f + "}";
    ...
    Blockly.Dart.procedures_callreturn = function(a) {
        for (var b = Blockly.Dart.variableDB_.getName(a.getFieldValue("NAME"), Blockly.PROCEDURE_CATEGORY_NAME), c = [], d = a.getVars(), e = 0; e < d.length; e++)
            c[e] = Blockly.Dart.valueToCode(a, "ARG" + e, Blockly.Dart.ORDER_NONE) || "null";
    ...
    Blockly.Dart.procedures_ifreturn = function(a) {
        ...
        a.hasReturnValue_ ? (a = Blockly.Dart.valueToCode(a, "VALUE", Blockly.Dart.ORDER_NONE) || "null",
    ...
    Blockly.Dart.text_charAt = function(a) {
        ...
        case "FROM_START":
            return a = Blockly.Dart.getAdjusted(a, "AT"),
            [c + "[" + a + "]", Blockly.Dart.ORDER_UNARY_POSTFIX];
        case "LAST":
        case "FROM_END":
            return a = Blockly.Dart.getAdjusted(a, "AT", 1),
            b = Blockly.Dart.provideFunction_("text_get_from_end", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String text, num x) {", "  return text[text.length - x];", "}"]),
            [b + "(" + c + ", " + a + ")", Blockly.Dart.ORDER_UNARY_POSTFIX];
        case "RANDOM":
            return Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;",
            b = Blockly.Dart.provideFunction_("text_random_letter", ["String " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(String text) {", "  int x = new Math.Random().nextInt(text.length);", "  return text[x];", "}"]),
    ...
    Blockly.Dart.text_prompt_ext = function(a) {
        ...
        "NUMBER" == a.getFieldValue("TYPE") && (Blockly.Dart.definitions_.import_dart_math = "import 'dart:math' as Math;",
