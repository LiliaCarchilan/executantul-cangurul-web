...
;(function(root, factory) {
    ...
    Blockly.Lua.finish = function(a) {
        var b = [], c;
        for (c in Blockly.Lua.definitions_)
            b.push(Blockly.Lua.definitions_[c]);
    ...
    Blockly.Lua.scrub_ = function(a, b, c) {
        ...
        if (!a.outputConnection || !a.outputConnection.targetConnection) {
            ...
            e && (e = Blockly.utils.string.wrap(e, Blockly.Lua.COMMENT_WRAP - 3),
            d += Blockly.Lua.prefixLines(e, "-- ") + "\n");
            for (var f = 0; f < a.inputList.length; f++)
                a.inputList[f].type == Blockly.INPUT_VALUE && (e = a.inputList[f].connection.targetBlock()) && (e = Blockly.Lua.allNestedComments(e)) && (d += Blockly.Lua.prefixLines(e, "-- "))
        }
    ...
    Blockly.Lua.lists_getIndex = function(a) {
        ...
        if ("LAST" != c && "FROM_END" != c && "RANDOM" != c || d.match(/^\w+$/)) {
            ...
            a = e(d, c, a);
            if ("GET" == b)
                return [d + "[" + a + "]", Blockly.Lua.ORDER_HIGH];
            c = "table.remove(" + d + ", " + a + ")";
    ...
    Blockly.Lua.controls_for = function(a) {
        ...
        if (Blockly.isNumber(c) && Blockly.isNumber(d) && Blockly.isNumber(e))
            ...
            a = "",
            g = Blockly.Lua.variableDB_.getDistinctName(b + "_inc", Blockly.VARIABLE_CATEGORY_NAME),
            a += g + " = ",
    ...
    Blockly.Lua.math_single = function(a) {
        ...
        if ("NEG" == b)
            return a = Blockly.Lua.valueToCode(a, "NUM", Blockly.Lua.ORDER_UNARY) || "0",
            ["-" + a, Blockly.Lua.ORDER_UNARY];
        if ("POW10" == b)
            return a = Blockly.Lua.valueToCode(a, "NUM", Blockly.Lua.ORDER_EXPONENTIATION) || "0",
    ...
    Blockly.Lua.procedures_defreturn = function(a) {
        var b = Blockly.Lua.variableDB_.getName(a.getFieldValue("NAME"), Blockly.PROCEDURE_CATEGORY_NAME)
          , c = "";
        Blockly.Lua.STATEMENT_PREFIX && (c += Blockly.Lua.injectId(Blockly.Lua.STATEMENT_PREFIX, a));
        ...
        for (var h = [], l = a.getVars(), k = 0; k < l.length; k++)
            h[k] = Blockly.Lua.variableDB_.getName(l[k], Blockly.VARIABLE_CATEGORY_NAME);
        c = "function " + b + "(" + h.join(", ") + ")\n" + c + d + e + g + f + "end\n";
    ...
    Blockly.Lua.procedures_callreturn = function(a) {
        for (var b = Blockly.Lua.variableDB_.getName(a.getFieldValue("NAME"), Blockly.PROCEDURE_CATEGORY_NAME), c = [], d = a.getVars(), e = 0; e < d.length; e++)
            c[e] = Blockly.Lua.valueToCode(a, "ARG" + e, Blockly.Lua.ORDER_NONE) || "nil";
    ...
    Blockly.Lua.procedures_ifreturn = function(a) {
        ...
        a.hasReturnValue_ ? (a = Blockly.Lua.valueToCode(a, "VALUE", Blockly.Lua.ORDER_NONE) || "nil",
    ...
    Blockly.Lua.text_join = function(a) {
        ...
        for (var c = 0; c < a.itemCount_; c++)
            b[c] = Blockly.Lua.valueToCode(a, "ADD" + c, Blockly.Lua.ORDER_NONE) || "''";
    ...
    Blockly.Lua.text_charAt = function(a) {
        ...
        if ("RANDOM" == b)
            b = Blockly.Lua.provideFunction_("text_random_letter", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str)", "  local index = math.random(string.len(str))", "  return string.sub(str, index, index)", "end"]),
            a = b + "(" + a + ")";
        else {
            if ("FIRST" == b)
                c = "1";
            ...
            c.match(/^-?\w*$/) ? a = "string.sub(" + a + ", " + c + ", " + c + ")" : (b = Blockly.Lua.provideFunction_("text_char_at", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str, index)", "  return string.sub(str, index, index)", "end"]),
            a = b + "(" + a + ", " + c + ")")
        }
        return [a, Blockly.Lua.ORDER_HIGH]
    }
    ...
    Blockly.Lua.text_changeCase = function(a) {
        ...
        if ("UPPERCASE" == b)
            ...
            "LOWERCASE" == b ? c = "string.lower" : "TITLECASE" == b && (c = Blockly.Lua.provideFunction_("text_titlecase", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str)", "  local buf = {}", "  local inWord = false", "  for i = 1, #str do", "    local c = string.sub(str, i, i)", "    if inWord then", "      table.insert(buf, string.lower(c))", '      if string.find(c, "%s") then', "        inWord = false", "      end", "    else", "      table.insert(buf, string.upper(c))", "      inWord = true", "    end", "  end", "  return table.concat(buf)", "end"]));
        return [c + "(" + a + ")", Blockly.Lua.ORDER_HIGH]
    }
    ;
